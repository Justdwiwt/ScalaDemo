...
object Solution_402 {
  ...
  private case class Zipper(left: List[Int], value: Int, right: List[Int]) {
    def toList: List[Int] = List(left.reverse, List(value), right).flatten
  }

  @scala.annotation.tailrec
  private def f(zipper: Zipper, k: Int): Zipper =
    if (k == 0) zipper match {
      case Zipper(Nil, 0, Nil) => Zipper(Nil, 0, Nil)
      case Zipper(Nil, 0, x :: xs) => f(Zipper(Nil, x, xs), 0)
      case z => z
    } else zipper match {
      case Zipper(Nil, _, Nil) => Zipper(Nil, 0, Nil)
      case Zipper(Nil, 0, x :: xs) => f(Zipper(Nil, x, xs), k)
      case Zipper(Nil, head, x :: xs) =>
        if (head > x) f(Zipper(Nil, x, xs), k - 1)
        else f(Zipper(List(head), x, xs), k)
      case Zipper(left :: ls, head, x :: xs) =>
        if (head >= left && head > x) f(Zipper(ls, left, x :: xs), k - 1)
        else f(Zipper(head :: left :: ls, x, xs), k)
      case Zipper(left :: ls, _, Nil) => f(Zipper(ls, left, Nil), k - 1)
    }


  def removeKdigits2(num: String, k: Int): String = num.map(_.asDigit).toList match {
    case Nil => "0"
    case x :: xs => f(Zipper(List.empty, x, xs), k).toList.mkString
  }
}

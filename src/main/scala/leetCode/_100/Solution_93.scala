...
object Solution_93 {
  ...
  private def pure[A](a: A): Parser[A] = Parser(l => List((a, l)))
  ...
  private def sat(p: Char => Boolean): Parser[Char] =
  ...
  private def many1[A](p: Parser[A]): Parser[List[A]] =
    p.flatMap(x => many(p).map(x :: _))

  private def many[A](p: Parser[A]): Parser[List[A]] = or(pure(Nil), many1(p))

...
object Solution_1036 {
  ...
  def f(blocks: Set[Cell], s: Cell, t: Cell): Boolean =
  ...
  def getNeighbours(p: Cell): Seq[Cell] = (-1 to 1).flatMap(i => (-1 to 1).map(j => (p._1 + i, p._2 + j)))

  def isInside(tl: Cell, br: Cell, p: Cell): Boolean =
  ...
  def wayOutsideExists(
    ...
    if (p.isEmpty) false
      ...
      else wayOutsideExists(blocks, np
        .filter(p => {

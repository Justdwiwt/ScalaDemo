...
object Solution_1765 {
  ...
  private def f(isWater: Array[Array[Int]], toVisit: Array[(Int, Int)], visited: Set[(Int, Int)], xMax: Int, yMax: Int): Unit = {
  ...
  def highestPeak(isWater: Array[Array[Int]]): Array[Array[Int]] = {
    ...
    val startPos = (0 to xMax).flatMap(x => (0 to yMax).map((x, _))).filter { case (x, y) => isWater(x)(y) == 1 }.toArray
    (0 to xMax).flatMap(x => (0 to yMax).map((x, _))).foreach { case (x, y) => if (isWater(x)(y) == 1) isWater(x)(y) = 0 else isWater(x)(y) = -1 }

...
object Solution_770 {
  ...
  def basicCalculatorIV(expression: String, evalvars: Array[String], evalints: Array[Int]): List[String] = {
    ...
    f(
      ...
      .map { case (s, c) => (c.toString +: s).mkString("*") }
      .toList
  }

  private def f(parsed: Parsed, expression: Seq[String], mulMode: Boolean = false): (Parsed, Seq[String]) = expression.headOption match {
    case None => (parsed, Seq.empty[String])
    case Some("+" | "-" | ")") if mulMode => (parsed, expression)
    case Some(")") => (parsed, expression.tail)
    case Some("+") =>
      val (rExp, expTail) = f(one, expression.tail, mulMode = true)
      f(sumExp(parsed, rExp), expTail)
    case Some("-") =>
      val (rExp, expTail) = f(one, expression.tail, mulMode = true)
      f(sumExp(parsed, rExp.mapValues(-_)), expTail)
    case _ if !mulMode =>
      val (rExp, expTail) = f(one, expression, mulMode = true)
      f(rExp, expTail)
    case Some("(") =>
      val (rExp, expTail) = f(zero, expression.tail)
      f(mulExp(parsed, rExp), expTail, mulMode = true)
    case Some(v) =>
      val nM = if (v.head.isLetter) Map(Seq(v) -> 1) else Map(Seq.empty[String] -> v.toInt)
      f(mulExp(parsed, nM), expression.tail, mulMode = true)
  }

...
object Solution_726 {
  ...
  case class E(l: Map[String, Int]) {
    def +(that: E): E =
      E(that.l.foldLeft(l) { case (map, (k, v)) => map + (k -> (v + map.getOrElse(k, 0))) })
  ...
  private def f(l: List[Char], acc: List[E]): List[E] = l match {
    case Nil => acc
    case '(' :: t => f(t, zero :: acc)
    case ')' :: t => f(t, update(acc, zero))
    case h :: t if h.isLetter => f(t.dropWhile(x => x.isLower | x.isDigit), getFirst(l) :: acc)
    case h :: t if h.isDigit => f(t.dropWhile(_.isDigit), (acc.head * getInt(l)) :: acc.tail)
    case List(_, _*) => acc
  }

  private def getInt(l: List[Char]): Int =
    l.takeWhile(_.isDigit).mkString.toInt

  private def getFirst(l: List[Char]): E = {
  ...
  def countOfAtoms(formula: String): String = f(formula.toList, Nil)
    .reduce(_ + _)
    .l
    .toSeq
    .sortBy(_._1)
    .map { case (k, 1) => k case (k, v) => k + v.toString }
    .mkString
}

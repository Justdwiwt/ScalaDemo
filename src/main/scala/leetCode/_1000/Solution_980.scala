  ...
  def uniquePathsIII(g: Array[Array[Int]]): Int = {
    case class Pos(r: Int, c: Int) {
      ...
      def inBounds: Boolean =
        g.indices.contains(r) && g.head.indices.contains(c)

      def peers: Array[Pos] = Array(Pos(r - 1, c), Pos(r, c + 1), Pos(r + 1, c), Pos(r, c - 1))
        .filter(p => p.inBounds && p.gval != -1)
    }

    val allPos = g.indices.flatMap(r => g(r).indices.map(Pos(r, _)))

...
object Solution_1129 {
  def shortestAlternatingPaths(n: Int, c1: Array[Array[Int]], c2: Array[Array[Int]]): Array[Int] = {
    ...
    while (arr1.filterNot(_ == 10000).max.max(arr2.filterNot(_ == 10000).max) >= i) {
      (0 until n)
        .withFilter(m2.keySet.contains(_))
        .withFilter(arr1(_) == i)
        .foreach(m2(_).foreach(y => arr2(y) = arr2(y).min(i + 1)))
      (0 until n)
        .withFilter(m1.keySet.contains(_))
        .withFilter(arr2(_) == i)
        .foreach(m1(_).foreach(y => arr1(y) = arr1(y).min(i + 1)))
      i += 1
    }

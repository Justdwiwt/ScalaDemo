...
object Solution_803 {
  ...
  def diff = List(Point(-1, 0), Point(1, 0), Point(0, 1), Point(0, -1))

  def hitBricks(grid: Array[Array[Int]], hits: Array[Array[Int]]): Array[Int] = {
    var res = List.empty[Int]

    def inBound(p: Point): Boolean = grid.indices.contains(p.x) && grid(p.x).indices.contains(p.y)
    ...
    def dfs(l: List[Point], path: List[Point] = Nil): List[Point] = l match {
      ...
      case h :: t =>
        ...
        else dfs(diff.map(delta => delta + h).filterNot(path.contains) ++ t, h :: path)
    }

...
object Solution_2421 {
  def numberOfGoodPaths(vals: Array[Int], edges: Array[Array[Int]]): Int = {
    ...
    vals.zipWithIndex.sorted.foldLeft(vals.length, counter) { case ((res, counter), (v, a)) =>
      graph(a).foldLeft(res, counter) { case ((res, counter), b) =>
        val (rootA, rootB) = (uf.find(a), uf.find(b))
        if (vals(b) > v || rootA == rootB) (res, counter)
        else uf.parent(rootA) = rootB
        val newCounter = counter.updated(rootB, counter(rootB).updated(v, counter(rootB).getOrElse(v, 0) + counter(rootA)(v)))
